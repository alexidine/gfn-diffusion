"""The gated-ramp forgetting sensor, bwd/relative_under_rise150 (train.py
...
def test_written_once_300_steps_exist_regardless_of_cadence(stride):
    ...
    assert 'relative_under_rise150' not in _feed(m, [1.0] * n_short, stride)
    m2 = _bind()
    assert _feed(m2, [1.0] * (300 // stride), stride)['relative_under_rise150'] == pytest.approx(0.0)
...
def test_a_three_nat_step_reads_as_three_nats_on_the_ten_step_cadence():
    ...
    assert st['relative_under_rise150'] == pytest.approx(3.0)
...
def test_period_matched_oscillation_cancels():
    ...
    st = _feed(m, series, 10)
    assert abs(st['relative_under_rise150']) < 0.05
