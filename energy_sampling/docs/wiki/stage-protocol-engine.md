...
- **`coeff_schedule`**: `{coefficient: {target, steps, kind}}`, a step-clock ramp of a numeric `cfg:energy_config` coefficient (the box and reduction penalties are the intended ones) from its config value to `target` over `steps` train steps, `kind` `geometric` (default, multiplies) or `linear` (adds); it holds at `target`. `Stage._parse_coeff_schedule` validates the shape; `StageProtocol.energy_coeffs` computes the live value and anchors the clock at the stage's first evaluation, stamping `stage_ctrl['coeff_sched_entry']` so a requeued leg continues the ramp rather than restarting it, and `train.py::Modeller.set_energy_coeffs` writes the value onto the energy function every 10 steps. It works under any `balance` kind; the lexicographic controller's `anneal_coeffs` is the event-driven ramp of the same coefficients, and naming one coefficient in both raises at parse. The live value is logged as `energy_func//<coefficient>` and the progress as `protocol/coeff_sched_p_<coefficient>`. Stored replay rows keep the reward they were admitted with, so a ramp shows a transient of roughly one residence time while rows scored under the older coefficients turn over (observation, 2026-09-23).
