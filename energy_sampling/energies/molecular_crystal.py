...
class MolecularCrystal(BaseSet):
    ...
    def get_conditioning_tensor(self,
        ...
        #mol_batch.reset_sg_info(sg_to_sample)  # todo re-add
        conditions = torch.cat(conds, dim=1) if len(conds) > 0 else torch.zeros_like(log_T_tensor)

        return log_T_tensor.flatten(), sg_to_sample, conditions
        # return (mol_batch,
