...
CHANGES: tuple[Change, ...] = (
    ...
    Change(
        state=6,
        summary=(
            "`condition_block_m` moves from the two buffer blocks to the two "
            "loss-coefficient blocks: buffers.prior_buffer -> bwd_loss_coeffs, "
            "buffers.replay_buffer -> replay_loss_coeffs. It never configured a "
            "store -- the buffer holds the same rows either way -- it shapes the "
            "DRAW for one loss, and both read sites were already gated on that "
            "loss's own coefficients (bwd on vg_lb; replay on vg_by_condition "
            "plus vg_lb/vg_lme). So the value only ever meant anything in "
            "conjunction with coefficients living in a different block, and a "
            "reader of either block alone could not tell whether it was active. "
            "THE MOVE ALSO CHANGES ITS SCOPE, which is the point rather than a "
            "side effect: loss coefficients are per-stage overridable "
            "(protocol.coeffs merges stage `loss_coeffs` over the base block), "
            "so a protocol can now run blocked draws in the VarGrad stage and "
            "independent ones elsewhere by declaring it. The vg_lb gate was "
            "emulating exactly that with the one stage-scoped signal it had. "
            "Value and semantics are unchanged: >= 2 blocks the draw, 0/1 does "
            "not, and the activation gates are untouched."
        ),
        components=('train.py', 'utils.py', 'config_invariants.py',
                    'configs/mk_dev.yaml'),
        transition=Transition(
            # Pure renames: same meaning, same units, value carried across. The
            # destination block always exists in a loadable config (train.py reads
            # all three *_loss_coeffs unconditionally), so no `added` default is
            # needed to make the target well-formed.
            renamed={
        ...
        invariants=(
            "A knob that is inert unless a loss coefficient is nonzero lives "
            "with that coefficient, so presence and activation are readable in "
            "one place.",
            "The value is read as an INT at the draw site: "
            "_sample_condition_blocked_indices passes it to np.random.choice's "
            "`size`, which rejects a float. Loss-coefficient blocks are written "
            "in floats throughout, so `2.0` is now a spelling a config can "
            "reach and the cast is load-bearing rather than defensive.",
            "vargrad_needs_groups reads it through config_invariants._coeff, the "
            "same resolver as `repeats`, so the rule sees a stage override "
            "instead of only the base block.",
        ),
        validation=(
            "test_config_state.py: the retired-key gate and the transitions "
            "describe the same set (both old paths retired, both renamed), and "
            "the canonical config migrates clean at the new version.",
            "test_config_invariants.py: the bwd disjunction mutation test now "
            "drives the coefficient, and its aug13/aug14 spellings still "
        ...
        transition=Transition(
            removed={
                ...
                'bwd_loss_coeffs.detach_pb':
                    'never read by any .py file a training step executes, so a '
                    'config setting it to 1 recorded an intention and ran the '
                    'un-detached branch. Dropped rather than migrated: there is '
                    'no behaviour to carry. P_B is controlled by '
                    'freeze_backward_policy and the freeze_pb/unfreeze_pb stage '
                    'actions.',
                'replay_loss_coeffs.detach_pb':
                    'the replay branch\'s copy of the same unread key -- see '
                    'bwd_loss_coeffs.detach_pb.',
            },
            moved={
                'adaptive_lr':
