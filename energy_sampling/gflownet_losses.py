...
def get_gfn_forward_loss(loss_coeffs,
    ...
    if loss_coeffs.var > 0:  # exclude abc-dimension - density goes crazy sometimes
        ...
        losses.append((overlap_loss + var_loss) * loss_coeffs.var)

    if loss_coeffs.buffer > 0:  # exclude c-dimension as density is genuinely uni-modal
        states_to_compare = states[:, -1, 3:].clip(min=-6, max=6)
