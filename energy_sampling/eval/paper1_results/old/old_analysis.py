...
from energy_sampling.eval.paper1_results.figures import parity_fig, sample_summary_table, var_table
from energy_sampling.eval.paper1_results.utils import get_gfn_samples, \
    generator_reward, get_rmsdmat, analyze_samples, new_local_analysis, get_kinetic_basins, get_kinetic_basins_sparse
...
if __name__ == '__main__':
    ...
    #
    # dmat = rmsdmat
    basins = get_kinetic_basins_sparse(sample_energy, sample_batch, kT, rmsdmat, samples,
                                alpha=5,
                                k_rad=10000,
                                n_points=10,
                                max_step_length=3.5)
    umap_model = UMAP(n_components=2, n_neighbors=50, min_dist=0.01, metric='precomputed')
    sample_embedding = umap_model.fit_transform(rmsdmat.to_dense().numpy().astype(np.float64))
    ...
    fig_dict = {}
    fig = sample_summary_table(sample_metrics, d_cut, sample_energy)
    fig_dict[f'thermo_table'] = fig
    fig = var_table(sample_metrics, d_cut, sample_energy)
