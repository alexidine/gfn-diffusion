...
def main():
    ...
    #: from config, which points at std_opt_ -- the polymorphs carrying the SAME
    #: conformer as our priors and our L1. Comparing against the other file
    #: compares a different molecule; see config.yaml.
    from energy_sampling.utils import load_yaml
    cfg = load_yaml(os.path.join(os.path.dirname(__file__), 'config.yaml'))
    print(f"polymorph reference: {os.path.basename(cfg['polymorphs'])}")
