...
def log_partition_function(initial_state, gfn, log_reward_fn, condition=None):
    states, log_pfs, log_pbs, log_fs = gfn.get_trajectory_fwd(initial_state, None, condition)
...
def mean_log_likelihood(data, gfn, log_reward_fn, num_evals=10, condition=None, energy=None):
    ...
    states, log_pfs, log_pbs, log_fs = gfn.get_trajectory_bwd(data, None, condition, energy)
